...
strategy_output_t track_strategy_step(float line_error, line_bin_t bin, imu_state_t imu, encoder_state_t enc, uint32_t tick_ms, const map_event_t *current_event) {
    ...
    if (line_lost(bin)) {
        ...
        if (runtime_feature_enabled(RC_FEATURE_LOST_PROTECTION) && (tick_ms - g_lost_since_ms) > LINE_LOST_TIMEOUT_MS) {
    ...
    if (runtime_feature_enabled(RC_FEATURE_MAP_PREDICTION) && line_error < STRAIGHT_ERROR_TH && line_error > -STRAIGHT_ERROR_TH &&
